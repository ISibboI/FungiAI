...
GamePanel::GamePanel(Game *game, Controller* enemy) :
        ...
        game(game),
        enemy(enemy) {
    auto *game_view_layout = new QGridLayout;
    ...
    game_scroll_widget = new QWidget;
    game_scroll_widget->setLayout(game_view_layout);

    game_scroll_area = new QScrollArea;
    ...
    auto *game_controls_layout = new QGridLayout;
    ...
    QWidget *game_controls_widget = new QWidget;
    game_controls_widget->setLayout(game_controls_layout);
    game_controls_widget->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Minimum);

    auto *turn_information_layout = new QGridLayout;
    ...
    layout->addWidget(game_scroll_area, 0, 0, 1, 3);
    layout->addWidget(game_controls_widget, 1, 1, 1, 1, Qt::AlignBottom);
    layout->addLayout(turn_information_layout, 2, 0, 1, 3, Qt::AlignBottom);
...
void GamePanel::game_finished_actions() {
    string text = game->winner() ? "You won!" : "You lost!";
    gui_state_label.setText(QString::fromStdString(text));
    action_target_label.setText(QString());
    target = 255;
    action_consequence_label.setText(QString());
    gui_state = GuiState::FINISHED;
}
...
void GamePanel::handle_pick_button() {
    if (gui_state >= GuiState::DISCARD_1 && gui_state <= GuiState::DISCARD_4 || gui_state == GuiState::FINISHED) { return; }
...
void GamePanel::handle_confirm_button() {
    switch (gui_state) {
        ...
        case GuiState::ACTION_PICK: {
            ...
            if (gui_state != GuiState::READY) {
                gui_state_label.setText(
                        QString::fromStdString("Discard %1 cards").arg(ignore_discard_action.get_discard_count()));
                action_target_label.setText(QString());
                target = 255;
                action_consequence_label.setText(QString());
                update_game_state();
    ...
    if (game->is_finished()) {
        game_finished_actions();
    }

    enemy->do_turn(*game, !current_player);
    game->post_turn_actions();
    update_game_state();
...
void GamePanel::handle_card_id_selected(uint8_t id) {
    if (gui_state != GuiState::READY && gui_state != GuiState::FINISHED) {
        target = id;
